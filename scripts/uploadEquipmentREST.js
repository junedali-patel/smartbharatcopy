// Frontend-compatible equipment upload script using Firestore REST API
const FIREBASE_PROJECT_ID = "smartbharat-32b65";
const FIREBASE_API_KEY = "AIzaSyB1zzxHkq1DZ9W2NkDPFP2qlYlCxF4z_60";
...
async function uploadEquipment() {
  ...
  for (const equipment of equipmentData) {
    try {
      const url = `https://firestore.googleapis.com/v1/projects/${FIREBASE_PROJECT_ID}/databases/(default)/documents/equipment?key=${FIREBASE_API_KEY}`;
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          fields: {
            name: { stringValue: equipment.name },
            description: { stringValue: equipment.description },
            category: { stringValue: equipment.category },
            dailyRate: { integerValue: equipment.dailyRate },
            location: { stringValue: equipment.location },
            city: { stringValue: equipment.city },
            state: { stringValue: equipment.state },
            phone: { stringValue: equipment.phone },
            imageUrl: { stringValue: equipment.imageUrl },
            status: { stringValue: equipment.status },
            rating: { doubleValue: equipment.rating },
            totalBookings: { integerValue: equipment.totalBookings },
            specifications: {
              mapValue: {
                fields: Object.entries(equipment.specifications).reduce((acc, [key, value]) => {
            ...
            userId: { stringValue: "system_admin" },
      ...
      if (response.ok) {
        successCount++;
        console.log(`✅ ${successCount}. Uploaded: ${equipment.name} (₹${equipment.dailyRate}/day)`);
      } else {
        const error = await response.json();
        failCount++;
        console.error(`❌ Failed to upload ${equipment.name}:`, error.error?.message);
      }
    } catch (error) {
      failCount++;
      console.error(`❌ Error uploading ${equipment.name}:`, error.message);
    }
  }

  console.log(`\n✨ Upload complete!`);
  console.log(`📊 Success: ${successCount}/${equipmentData.length}`);
  if (failCount > 0) {
    console.log(`❌ Failed: ${failCount}/${equipmentData.length}`);
  }
  console.log('📱 Equipment will now appear in the app.\n');
}

uploadEquipment().catch(console.error);
